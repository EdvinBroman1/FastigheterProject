...
#pragma warning disable CS1591
    ...
    public class RealEstatesController : ControllerBase
    {
        ...
        public IActionResult GetRealEstates(int realEstateId)
        {
            ...
            if (User.Identity.IsAuthenticated) return Ok(_mapper.Map<RealEstateDto>(realEstateFromRepo));

            return Ok(_mapper.Map<RealEstateDetailsDto>(realEstateFromRepo));
        }
        /// <summary>
        /// Get an Realestate by the username
        /// </summary>
        /// <param name="Username">Username of user who has created the Realestates</param>
        /// <returns>returns a enumerable of realestates with id, Title, Rentingprice, purchaseprice, canberented and can be purchased </returns>
        /// <response code="200">Gets the Requested Realestate</response>
        [HttpGet("byuser/{username}")]
        public IActionResult GetRealEstateByUserName(string Username)
        {
            var realEstateFromRepo = realEstateRepo.GetRealEstateByUser(Username);

            if (realEstateFromRepo == null) return NotFound();

            if (User.Identity.IsAuthenticated) return Ok(_mapper.Map<IEnumerable<RealEstateDto>>(realEstateFromRepo));
